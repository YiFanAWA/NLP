...
def load_biorelex_dataset(base_path, tokenizer):
    ...
    # 支持绝对路径和相对路径自动查找
    train_fp = os.path.abspath(join(base_path, 'train.json'))
    dev_fp = os.path.abspath(join(base_path, 'dev.json'))
    if not os.path.exists(train_fp):
        train_fp = os.path.join(os.path.dirname(__file__), '..', 'resources', 'biorelex', 'train.json')
    if not os.path.exists(dev_fp):
        dev_fp = os.path.join(os.path.dirname(__file__), '..', 'resources', 'biorelex', 'dev.json')
