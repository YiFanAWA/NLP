...
def prepare_configs(config_name, dataset, split_nb, verbose=True):
    ...
    # save_dir (only create when save_artifacts is enabled)
    configs['save_dir'] = join(BASE_SAVE_PATH, '{}_{}'.format(configs['dataset'], config_name))
    # By default we avoid creating project-wide save directories to prevent
    # automatic generation of checkpoints or other artifacts. To enable
    # artifact creation, set `save_artifacts = true` in the config or pass
    # a runtime flag that sets `configs['save_artifacts'] = True`.
    if configs.get('save_artifacts', False):
