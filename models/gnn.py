...
class CompGCN(nn.Module):
    ...
    def build_graph(self, fw_adjs):
        ...
        # 强制图和边类型在 CPU 上
        g = dgl.graph((src, tgt)).to('cpu')
        edge_type = torch.tensor(lst).to('cpu')
        return g, edge_type
    ...
    def forward(self, x, fw_adjs, bw_adjs=None):
        ...
        if g.number_of_nodes() == len(x_cpu):
            r = self.init_rel.to('cpu')
            for layer in self.layers:
                x_cpu, r = layer(g, x_cpu, r, edge_type, edge_norm)
        # 输出再转回主模型设备
        return x_cpu.to(self.device)
...
class Gnn(nn.Module):
    ...
    def forward(self, candidate_embs): # [n, h=512]
        # If this GNN instance is intended to run on CPU, move inputs and modules to CPU
        if getattr(self, 'device', None) == 'cpu':
            candidate_embs = candidate_embs.to('cpu')
            try:
                self.relation_scorer.to('cpu')
            except Exception:
                pass
        ...
        # If GNN runs on CPU but main model expects device elsewhere, move output back
        if getattr(self, 'device', None) == 'cpu':
            try:
                return ieg_out_h.to(candidate_embs.device if candidate_embs.device.type != 'cpu' else 'cpu')
            except Exception:
                return ieg_out_h
        return ieg_out_h
    ...
    def adjs_from_preds(self, relation_probs):
        ...
        for ix in range(len(self.ieg_etypes)):
            ...
            # Ensure adjacency matrices are on CPU so DGL graph ops don't try CUDA
            fw_adjs.append(A.to('cpu'))
            bw_adjs.append(A.T.to('cpu'))
