...
# Note: import Gnn lazily in __init__ to avoid requiring DGL at import-time
...
class JointModel(BaseModel):
    def __init__(self, configs):
        ...
        # using gnn (bigcn and compgcn)
        # Import and instantiate Gnn lazily so code can run without DGL installed
        self.use_gnn = self.configs['use_gnn']
        if self.use_gnn:
            try:
                from models.gnn import Gnn
                # Respect `gnn_force_cpu` config: when true, instantiate GNN on CPU
                if configs.get('gnn_force_cpu', True):
                    self.gnn = Gnn(configs, 'cpu')
                else:
                    # instantiate on the model device
                    self.gnn = Gnn(configs, self.device)
            except Exception as e:
                # If GNN import fails, disable GNN and warn (keeps training runnable)
                print('Warning: failed to import Gnn (DGL may be missing). Disabling GNN. Error:', e)
                self.use_gnn = False

        # Move to Device
        self.to(self.device)
        # If GNN is configured to force CPU, ensure it stays on CPU after moving whole model
        if self.use_gnn and configs.get('gnn_force_cpu', True):
            try:
                self.gnn.to('cpu')
                self.gnn.device = 'cpu'
            except Exception:
    ...
    def forward(self, input_ids, input_masks, mask_windows,
        ...
        candidate_embs = self.span_relu(self.span_linear_2(candidate_embs))# [500, 512]

        if self.use_gnn and len(candidate_embs)<600:
            # GNN runs on CPU internally; ensure its output is moved to the main model device
            candidate_embs =  self.gnn(candidate_embs).to(self.device)
